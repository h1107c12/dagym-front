...
const Page = styled(SafeAreaView)`
  flex: 1;
  background: ${(p: TTheme) => p.theme.colors.background};
`;
...
/* ===== Hero (LinearGradient with typed attrs) ===== */
...
/* ===== Progress Bar (typed props explicitly) ===== */
type ProgressFillProps = { w: number; color?: string } & TTheme;
...
const ProgressFill = styled.View<ProgressFillProps>`
  height: 100%;
  width: ${(p: ProgressFillProps) => `${p.w}%`};
  border-radius: 999px;
  background: ${(p: ProgressFillProps) => p.color ?? p.theme.colors.primary};
`;
...
export default function ReportScreen() {
  ...
  const weightPts = useMemo(() => {
    ...
    return weeklyData.map((d, i) => ({
      x: sx(i, weeklyData.length, wChartW),
      y: sy(d.weight, wMin, wMax),
    }));
  }, [wChartW]);
      ...
      <Container>
        ...
        <Hero>
          ...
          <View style={{ flexDirection: 'row', gap: 24, marginTop: 8 }}>
            <View>
              <HeroBig>-2.3kg</HeroBig>
              <HeroMeta>체중 감량</HeroMeta>
            </View>
            <View>
              <HeroBig>18일</HeroBig>
              <HeroMeta>운동 일수</HeroMeta>
            </View>
          </View>
        </Hero>
        ...
        <Grid>
          <Card>
            ...
            <Sub>
              목표: {monthlyStats.weightLoss.target}
              {monthlyStats.weightLoss.unit}
            </Sub>
            <ProgressTrack>
              <ProgressFill
                w={(monthlyStats.weightLoss.value / monthlyStats.weightLoss.target) * 100}
              />
            </ProgressTrack>
          </Card>
          ...
          <Card>
            ...
            <Sub>
              목표: {monthlyStats.avgCalories.target}
              {monthlyStats.avgCalories.unit}
            </Sub>
            <ProgressTrack>
              <ProgressFill
                w={(monthlyStats.avgCalories.value / monthlyStats.avgCalories.target) * 100}
              />
            </ProgressTrack>
          </Card>
          ...
          <Card>
            ...
            <Sub>
              목표: {monthlyStats.workoutDays.target}
              {monthlyStats.workoutDays.unit}
            </Sub>
            <ProgressTrack>
              <ProgressFill
                w={(monthlyStats.workoutDays.value / monthlyStats.workoutDays.target) * 100}
              />
            </ProgressTrack>
          </Card>
          ...
          <Card>
            ...
            <Sub>
              목표: {monthlyStats.bodyFat.target}
              {monthlyStats.bodyFat.unit}
            </Sub>
            <ProgressTrack>
              <ProgressFill
                w={(monthlyStats.bodyFat.value / monthlyStats.bodyFat.target) * 100}
                color="#ef4444"
              />
            </ProgressTrack>
          </Card>
            ...
            {wChartW > 0 && (
              <Svg width={wChartW} height={CHART_H}>
                ...
                <AnimatedPolyline
                  ...
                  // Animated types for react-native-svg aren't perfect; cast to any
            ...
            {eChartW > 0 && (
              <Svg width={eChartW} height={CHART_H}>
                ...
                {weeklyData.map((d, i) => {
                  ...
                  const yAnim = barProgress[i].interpolate({
                    inputRange: [0, 1],
                    outputRange: [yBase, yTop],
                  });
                  const hAnim = barProgress[i].interpolate({
                    inputRange: [0, 1],
                    outputRange: [0, hTarget],
                  });
                ...
                <G>
                  {weeklyData.map((d, i) => (
                    <SvgText
                      key={i}
                      x={sx(i, weeklyData.length, eChartW)}
                      y={CHART_H - PAD.b + 16}
                      fill="#9aa0a6"
                      fontSize={12}
                      textAnchor="middle"
            ...
            {[
              ...
              <View
                ...
                style={{
                  ...
                  padding: 12,
                  borderWidth: 2,
                  borderColor: a.done ? '#bbf7d0' : '#e5e7eb',
                  backgroundColor: a.done ? '#f0fdf4' : '#f9fafb',
                }}
              >
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  <View style={{ width: 24, alignItems: 'center' }}>
                    <Sub style={{ fontSize: 18 }}>{a.icon}</Sub>
                  </View>
                  <View style={{ flex: 1 }}>
        ...
        <CoachCard>
          ...
          <View style={{ marginTop: 6 }}>
            <Sub>
              <Sub style={{ fontWeight: 'bold', color: '#065f46' }}>잘하고 있어요!</Sub> 이번 주
              운동 참여율이 85%로 목표를 달성했습니다.
            </Sub>
            <Sub style={{ marginTop: 6 }}>
              체중이 꾸준히 감소하고 있어요. 현재 속도로 가면 목표 체중에 3주 후 도달 예상됩니다.
            </Sub>
            <Sub style={{ marginTop: 6 }}>
              <Sub style={{ fontWeight: 'bold', color: '#065f46' }}>개선 포인트:</Sub> 주말
              운동량이 부족해요. 가벼운 산책이나 스트레칭을 추천드립니다.
            </Sub>
          </View>
        </CoachCard>
      </Container>
